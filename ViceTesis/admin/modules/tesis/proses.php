...
//  el usuario ya ha iniciado sesión, a continuación, ejecutar el comando para insertar, actualizar y borrar
            ...
            if(in_array($extension, $allowed_extensions)) {
                ...
                if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                    ...
                    if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen fue subida o no con éxito
                        ...
                        $query = sqlsrv_query($mysqli, "INSERT INTO tesisBE(title,image)
        ...
        if (isset($_POST['save'])) {
            if (isset($_POST['portfolio_id'])) {
                ...
                if (empty($nama_file)) {
                    ...
                    $query = sqlsrv_query($mysqli, "UPDATE is_portfolio SET title        = '$title'
                                                                      WHERE portfolio_id = '$portfolio_id'")
                                                    or die('Ada kesalahan pada query update : '.sqlsrv_errors($mysqli)); 
                        ...
                        if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                            ...
                            if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen subida o no con éxito
                                ...
                                $query = sqlsrv_query($mysqli, "UPDATE is_portfolio SET title        = '$title',
                                                                                        image        = '$nama_file'
                                                                                  WHERE portfolio_id = '$portfolio_id'")
                                                                or die('e produjo un error en una consulta de actualización : '.sqlsrv_errors($mysqli)); ;
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['id'])) {
            ...
            $query = sqlsrv_query($mysqli, "DELETE FROM is_portfolio WHERE portfolio_id='$portfolio_id'")
