...
//  el usuario ya ha iniciado sesión, a continuación, ejecutar el comando para insertar, actualizar y borrar
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['save'])) {
            ...
            $contenido             = mysqli_real_escape_string($mysqli, trim($_POST['contenido']));
            ...
            if(in_array($extension, $allowed_extensions)) {
                ...
                if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                    ...
                    if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen fue subida o no con éxito
                        ...
                        $query = mysqli_query($mysqli, "INSERT INTO is_revista(tittle,contenido,image)
                                                        VALUES('$title','$contenido','$nama_file')")
    ...
    elseif ($_GET['act']=='update') {
        if (isset($_POST['save'])) {
            if (isset($_POST['revista_id'])) {
                // Captura de datos desde el formulario
                $revista_id       = mysqli_real_escape_string($mysqli, trim($_POST['revista_id']));
                $title              = mysqli_real_escape_string($mysqli, trim($_POST['title']));
                $contenido          = mysqli_real_escape_string($mysqli, trim($_POST['contenido']));
                ...
                if (empty($nama_file)) {
                    // consultar comando para cambiar los datos de la tabla is_revista
                    $query = mysqli_query($mysqli, "UPDATE is_revista SET tittle = '$title',contenido='$contenido'
                    ...
                    if(in_array($extension, $allowed_extensions)) {
                        ...
                        if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                            ...
                            if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen subida o no con éxito
                                // Si la imagen ha cargado correctamente, Marca:
                                // consultar comando para cambiar los datos sobre la tabla is_revista
                                $query = mysqli_query($mysqli, "UPDATE is_revista SET tittle        = '$title',
                                                                                     contenido        = '$contenido',
                                                                                        image        = '$nama_file'
                                                                                  WHERE revista_id = '$revista_id'")
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['id'])) {
            ...
            $query = mysqli_query($mysqli, "DELETE FROM is_revista WHERE revista_id='$revista_id'")
